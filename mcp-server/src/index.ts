...
server.tool(
  "list_invoices",
  "List ZATCA invoices with optional filters. Returns paginated invoice summaries including status, totals, and ZATCA submission results.",
  {
    status: z
      .string()
      .optional()
...
server.tool(
  "get_invoice",
  "Get full invoice detail including line items, ZATCA submission status, error messages, QR code, and the event timeline showing every step of the compliance pipeline.",
  {
...
server.tool(
  "create_invoice",
  "Create a Saudi e-invoice and run the available ZATCA pipeline. Before calling, use get_zatca_status and confirm environment='production' for a legally submitted invoice; sandbox submissions are tests only. Standard (B2B) invoices require buyerName, buyerVatNumber, and buyerCity and normally clear; simplified invoices normally report. Always inspect the returned status: cleared/reported means submitted, generated/signed/pending means not yet submitted, needs_data/awaiting_buyer_data requires correction, and failed requires action.",
  {
    invoiceType: z
  ...
  async ({
    ...
    for (const [index, line] of lines.entries()) {
      ...
      if (
        (line.vatCategory === "Z" || line.vatCategory === "E" || line.vatCategory === "O") &&
        !line.vatexCode
      ) {
...
server.tool(
  "create_credit_note",
  "Issue a ZATCA credit note (document type 381) against a cleared or reported invoice. Omit lines for the remaining full refundable balance, or provide positive partial-refund lines that do not exceed the parent's remaining quantities and amounts. Supply a stable idempotencyKey so agent/network retries cannot create a duplicate credit note.",
  {
...
server.tool(
  "retry_invoice",
  "Re-run the ZATCA compliance pipeline for a failed invoice after correcting it in the Saudi VAT Pro dashboard. Returns the updated status; always inspect it rather than assuming submission succeeded.",
  {
...
server.tool(
  ...
  "Get an AI-generated plain-language explanation of why an invoice failed ZATCA validation, along with specific suggested actions to fix it. Most useful for 'failed' or 'needs_data' invoices.",
  {
...
server.tool(
  "get_zatca_status",
  "Check certificate (CSID) enrollment, expiry, environment, and readiness before creating invoices. environment='sandbox' is test-only and does not constitute legal ZATCA submission; confirm environment='production' for live invoices.",
  {},
  async () => {
...
server.tool(
  ...
  "Get the merchant's business profile including business name, VAT number, address, default invoice settings, and subscription plan.",
  {},
  async () => {
...
server.tool(
  ...
  "Get a compliance health summary: total invoices, counts by status (cleared, failed, pending), VAT totals for the current period, and any outstanding issues requiring attention.",
  {},
  async () => {
